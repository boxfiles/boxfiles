---
name: crustjs-config
description: A skill for managing local JSON configuration files in CLI apps, with platform-aware paths and typed access. Use when you need to persist user preferences, app state, or cached data across runs of a CLI tool.
---
...
### `store.write(state)` [#storewritestate]
...
### `store.update(updater)` [#storeupdateupdater]
...
### `store.patch(partial)` [#storepatchpartial]
...
#### Schema-derived defaults and TypeScript [#schema-derived-defaults-and-typescript]
...
#### `field()` throws on invalid input [#field-throws-on-invalid-input]
